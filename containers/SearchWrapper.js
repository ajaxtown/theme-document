...
export default class SearchWrapper extends Component {
  ...
  loadData = async ({ query, type }) => {
    ...
    if (term === "post") {
      ...
      this.setState({
        loading: false,
        posts: [...this.state.posts, ...result.data.postsByTaxSlug.posts],
  ...
  render() {
    ...
    if (!this.state.isSearch) {
      return (
        <div className="post-row p-t-30 card content">Start your search...</div>
      );
    }
    if (posts.length === 0) {
      return (
    ...
    const data = (
      <div className="post-row col-lg-8 col-lg-offset-2 content">{posts}</div>
    );
    ...
    return (
      <React.Fragment>
        ...
        {config.itemsPerPage < this.state.total && (
          <Paginate count={this.state.total} match={this.props.match} />
        )}
      </React.Fragment>
    );
  }
